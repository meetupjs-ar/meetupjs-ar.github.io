...
},{"whatwg-fetch":4}],2:[function(require,module,exports){
...
function renderMonthlyCalendars(monthlyCalendar) {
    return new Promise(function (resolve, reject) {
        try {
            monthlyCalendar.forEach(function (calendar) {
                var gridId = 'grid-' + calendar.when.month + '-' + calendar.when.year;
                var weekdaysId = 'weekdays-' + calendar.when.month + '-' + calendar.when.year;

                calendarElement.insertAdjacentHTML('beforeend', '<div class="fadeIn mv5">\n                        <h2 class="black-alternative f2 mb4 mt0 normal tc ttc">\n                            ' + calendar.when.month + ' ' + calendar.when.year + '\n                        </h2>\n                        <div id="' + weekdaysId + '"\n                            class="b--black-30 bl bt dn flex-l">\n                        </div>\n                        <div id="' + gridId + '"\n                            class="b--black-30 br bt flex flex-wrap">\n                        </div>\n                    </div>');

                var weekdaysElement = calendarElement.querySelector('#' + weekdaysId);

                moment.weekdays().forEach(function (weekday) {
                    weekdaysElement.insertAdjacentHTML('beforeend', '<div class="b--black-30 bg-white black-50 br pv3 tc ttc w-one-seventh-l">\n                            ' + weekday + '\n                        </div>');
                });

                var gridElement = calendarElement.querySelector('#' + gridId);
                var monthNumber = parseInt(moment().month(calendar.when.month).format('MM')) - 1;
                ...
                if (currentMonth.isoWeekday() !== 7) {
                    for (var i = currentMonth.isoWeekday(); i > 0; i--) {
                        gridElement.insertAdjacentHTML('beforeend', '<div class="b--black-30 bb bg-near-white bl dn db-l w-one-seventh-l">\n                            </div>');
                    }
                }

                var today = moment();

                for (var _i = 1; _i <= currentMonth.daysInMonth(); _i++) {
                ...
                calendar.events.forEach(function (event) {
                    ...
                    var list = gridElement.querySelector('#cell-' + eventDay.format('DDMMYY'));
                    var counter = list.nextElementSibling;
...
}).then(renderMonthlyCalendars).catch(handleError);

},{"isomorphic-fetch":1,"moment":3,"moment/locale/es":2}]},{},[5]);
